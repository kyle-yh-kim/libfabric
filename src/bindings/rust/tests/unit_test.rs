...
mod unit_tests {
    ...
    /// Macro to generate linkage tests for libfabric functions
    /// This macro creates a test function that verifies a function pointer is not null
    ...
    /// Test core libfabric API function linkage
    /// These tests verify that the function pointers are properly linked
    /// without actually calling the functions (which would require initialization)
    ...
    /// Test structure definitions are properly bound
    /// Test that we can create instances of key structures.
    #[test]
